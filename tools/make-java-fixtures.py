...
TARGETS = [
    ...
    Target(
        ...
        why="Rhenium Dust, and the claim is about the CODE PATH rather than about the pack: a "
            ...
            "The conclusion was an artifact of a broken search rather than a fact about the "
            "pack. Two things separated them: a positive control (the ten gated ores planned "
            "directly must show a gate at the root; 9 of 10 do, the tenth being #270), and "
            "the reverse-reachability population rather than a random sample -- a key can "
            "only reach a gate if it transitively depends on a gated ore, and sampling the "
            "keyspace for a rare property makes a true negative and a missed one identical. "
            "One of the probes written for this check itself printed a confident `0 of 10` "
            "because it called `.to_json()` on a dict and every plan raised; a broken probe "
            "and a real negative look the same in the output, and only the control told them "
            "apart.\n"
            "\n"
