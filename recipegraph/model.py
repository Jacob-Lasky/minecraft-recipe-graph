...
class Graph:
    ...
    def real_producers(self, key):
        """`producers`, minus the recipes `real_production` says are not making `key`.
        ...
        THE NON-FLUID SHORT CIRCUIT IS AN OPTIMISATION THAT DEPENDS ON `real_production`
        EXCLUDING FLUIDS AND NOTHING ELSE. It hands back the memoised list itself with no
        allocation, which matters because the solver asks this of every candidate of every
        node. Widen `real_production` past the fluid case and this branch silently stops
        honouring it, so `AllThreeReadersAgreeTest` compares the two forms over every
        recipe of every fixture graph rather than trusting the reasoning here.
        """
        if not key.startswith(FLUID_PREFIX):
            return self.producers(key)
        return [r for r in self.producers(key) if self.real_production(r, key)]
