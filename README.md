...
| **The dump mod** | `mod/` — a client-side Forge jar adding `/recipedump` | the **client's** `mods/`, on the machine that plays | prebuilt as `dist/mc-recipe-dump-0.9.12.jar` |
...
```bash
cp dist/mc-recipe-dump-0.9.12.jar '/path/to/instance/minecraft/mods/'
```
